  ...
  run: async (client, interaction, args) => {
    if (!interaction.member.permissions.has(Discord.PermissionFlagsBits.ManageGuild)) {
      ...
      let embed = new Discord.EmbedBuilder()
        .setAuthor({ name: `Um novo sorteio está sendo realizado!`, iconURL: interaction.guild.iconURL({ dynamic: true }) })
...
> Tempo: \`${tempo}\`.
<a:confete:1057032005986037903> Clique no botão para parcipar.\n**Boa sorte!!!**`)
      ...
      let erro = new Discord.EmbedBuilder()
        .setColor("Red")
        .setDescription(`<a:errado:1053788035969454110>・Não foi possível promover o soteio!`);
      ...
      coletor.on("collect", (i) => {
        ...
        if (i.customId === "botao") {
          ...
          if (click.includes(i.user.id)) return i.reply({ content: `<a:errado:1053788035969454110>・Olá ${interaction.user}, você ja está participando do sorteio.`, ephemeral: true });

          click.push(i.user.id);

          interaction.editReply({ embeds: [embed] });

          i.reply({ content: ` <a:confete:1057032005986037903>・Olá ${interaction.user}, você entrou no sorteio.`, ephemeral: true });
        }
      ...
      setTimeout(() => {
        ...
        if (click.length == 0) return interaction.followUp(`\n<a:errado:1053788035969454110>・**SORTEIO CANCELADO!**\nNão houveram participantes no sorteio \`${premio}\`.`);

        interaction.followUp(`**<a:confete:1057032005986037903>・Parabéns <@${ganhador}> você ganhou o sorteio: \`${premio}\`.**`);
